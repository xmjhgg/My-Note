...
## 十一.接口：Unix环境下的用户接口设计模式
程序的接口就是程序同人类用户以及其他程序通讯的方法总和（不止是代码中的”接口“，还包括用户的行为交互）。  
在Unix接口设计中，主要由两个主题：与其他程序通讯的前瞻性设计；最小立异原则  
- 最小立异原则  
最小立异原则是所有接口设计的通用原则：”少来标新立异“。接口的设计最好能避免设计一种全新模式的接口，新颖其实也是一种进入门槛，给用户加上学习负担。当然这个原则不能被理解成在设计中号召机械的保守注意，新颖虽然提高了用户与接口最初的几次交互成本，但是一个糟糕的接口设计却让人更加痛苦，原有的设计规则如果并不能带来良好的品味和工程，那么可以选择颠覆他。但是通常情况下，接口的设计者总是不自觉地为用户着想时，聪明过了头。  
### 接口设计评估
作者将接口评估标准分为五个：简洁、表现力、易用、透明和脚本化能力  
- 简洁：指一个接口事务处理需要的动作时间以及复杂度有较低的上限。  
- 表现力：接口具有表现力是指接口可以触发相当广泛的行为。最具有表现力的接口可以产生设计者没有预见的行为组合，但是仍然可以给予用户有用和一致的结果。  
- 易用：指接口要求用户需要记忆的东西少，那么接口的易用性就越高。
- 透明：即Unix哲学中强调的透明度，用户在使用时，几乎没有什么程序的问题、数据或者状态需要记忆，所见即所得。就如之前笔记中有将到的一个声音编辑软件，接口透明度就很好。  
- 脚本能力：指接口能够很容易的为其他程序所使用，减少了定制代码的昂贵需求并使得自动化重复执行任务相对容易。
### Unix中的接口设计模式
这一节主要讲了在Unix中有使用的接口设计模式。  
- 过滤器模式：过滤器程序将接受标准的输入然后经过程序的转换再发送到标准的输出端。如：Unix中的tr、grep程序。  
过滤器模式下的接口附带的原则：  
1. 宽进严出：尽可能的宽松的接受输入，输出结构良好的严谨的数据。宽进是为了有更好的兼容性，严出是为了能更好的和其他程序交互。  
2. 过滤时，多余的信息也不丢弃。也是为了能更好的和其他程序交互，防止因为丢弃了本程序不需要的信息而导致后续需要这份信息的程序不能消费。  
3. 过滤时，不增加无用的数据。  
- Cantrip模式：最简单的设计模式，没有输入，没有输出，只被调用有一次并产生退出状态的值。如果程序再运行时只是需简单的事并且不需要和用户进行交互，那么Cantrip会是一种优秀的模式。  
- 源模式：不需要输入，输出的内容可以在程序启动时控制。如ls  
- 引擎和接口分离模式：一种分离单进程的设计模式，将程序定义核心算法与数据结构的”引擎“将和用户命令、显示结果等“接口”部分分离。如现在流行的MVC、前后端分离。  

### 沉默是金
Unix中的接口还有一个原则：如果这个程序没有什么好说的，那就应该闭嘴。理由有下：  
1. 喋喋不休的程序往往不能和其他程序很好的合作。其他的程序需要解析许多信息，有些信息的解析就会陷入到丢弃还是使用的两难的境地。  
2. 用户的屏幕空间和注意力是宝贵的。程序每产生一行垃圾信息，用户可见的有用信息就减少一行。  

## 十二.优化
这一章内容比较少，主要是强调如果系统上还没有什么性能问题，不妨先放着，“过早优化是万恶之源”。如果确实有性能优化上的需求，需要先定位到系统的瓶颈所在，在去进行优化。  

## 十三.复杂度：尽可能简单，但别简单过了头
在这篇笔记的开头，就描述了Unix哲学的概括：“Keepi It Simple”。尽可能地保持设计简单，但是什么样的设计是“尽可能地简单”，如何断定是否简单？这一个问题作者并没有给出明确的答案，而是在这一章阐述了什么是复杂度，复杂度的来源。也许对复杂度了解的更深入可以反过来帮助我们如何去理解“尽可能简单的设计”。  

### 什么是复杂度
复杂度对于不同人来说，有着不同的理解，那么如何判断一个软件的复杂度呢？  
- 从程序员的角度出发，他们更注重程序实现的复杂度，也就是程序员为了能够试图理解程序，从而建立其思维模型并调试这个程序的程度。越容易理解复杂度越低。  
- 从软件的最终用户出发，他们更注重接口复杂度，注重软件是否易用，也就是使用软件对用户的记忆负担，记忆负担越小，对用户来说复杂度越低。  
- 还有一种简单评判标准，程序的代码总量。更多的代码行数也意味着更多的bug，程序调试起来就越为麻烦。  

这三个复杂度：实现复杂度、接口复杂度、代码量并不是单独存在的，三者互相影响，甚至可能会同时上升。我们要做的防御复杂度的办法就是在这三者间进行权衡。  
如：我们选择放弃防御接口复杂度，降低实现复杂度和代码量，那么可能会导致将系统底层功能抛给用户。如某个编辑器软件缺少全局替换的功能，导致用户需要一个个的使用单个替换功能来完成工作。如果保持程序的代码量小，而采用了极为晦涩的实现方式来达到功能，则会导致系统的实现复杂度提高，代码可读性变差。

这就是复杂度的三个面孔，以及一些容易导致设计者掉进去的陷阱。  

### 本质的、选择的、偶然的复杂度
- 本质复杂度  
在理想的世界中，程序员都希望打造一个小巧简单的完美软件，但是现实世界中存在着太多复杂的问题需要用复杂的方式去解决，用十几行代码再优雅也不能控制飞机，飞机上又太多的设备、道路交通、系统、人员交互，一个飞机的控制系统必然是复杂的。这也就是本质复杂度，一个系统无法改变的本质上的复杂程度。  
- 偶然复杂度  
偶然复杂度，是指当时设计系统时没有发现实现功能的最简的方法，偶然产生的复杂度。  
- 选择复杂度  
设计系统时，选择了某种系统方法论或者是选择实现系统的多个功能而产生的复杂度，由后续人为设计系统时不同的选择产生的复杂度。  

**复杂度映射表**  
![](./img/fzdys.png)  

### 当简洁不能胜任
Unix中坚持简洁的传统，这种传统有时会诱使程序员产生错误的思想：似乎所有的复杂度都是偶然复杂度，甚至宁可去掉功能也不接受可能的复杂度。  
简洁的设计确实容易使程序员感觉良好，能防止软件表面光鲜但内在混乱的问题。但是**计算资源同人类的思考和财富一样，并不是依靠存储来体现价值，而是靠消费来证明价值**。我们需要注意什么时候设计上的简洁已经不能够提供价值了，而是一种伪装成软件设计苦行僧的外衣一样，以简洁为借口来敷衍工作的纵容方式时，请调整这个软件的设计理念。  
这个问题似乎又变成了支持放弃简洁为美的理由，但是过度的臃肿软件又会将我们推向毁灭，当我们面临这样的抉择时，也许是停下来好好思考的时候了。

## 十四及之后
书中剩余的几章是讨论Unix软件的实现部分，个人认为书中的部分想法可能对我没有太大的帮助（Javaer），这一部分就简单记录一下有一些帮助的知识。  
- 不要重新发明轮子  
  牢记经济原则，如果已经有现成的，就直接拿来用，即重用代码。自己重头写又费时又容易出现bug。（当然，这是在工作中的建议。如果是学习的话，重复造一遍轮子自己实现一遍会帮助我们更好的深入了解原理，提高计数）  
- 注重软件文档的编写  
- 开源项目并不简单，需要投入精力维护  
  并不是将代码放到公开的代码仓库即可，还要注意开源许可证的选择、维护项目代码风格、要维护社区的活跃、及早更新及早发布。  

# 总结
引用书中的Unix核心原则来概括这本书的中心思想：Keep It Simple,Stupid。无论是从用户界面、软件设计还是程序代码，尽量保持程序的简洁。把编程视为艺术，而这个艺术的核心就是：简洁与美。通过讲述Unix的各个设计思想以及多个相关软件实例，来带领大家深入理解编程的艺术：模块化、简洁性、可读性，体会编程的美。  
用追求艺术的态度来编程。